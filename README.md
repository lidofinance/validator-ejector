...
|-----------------------------------------------|----------|-----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| EXECUTION_NODE                                | Yes      | http://1.2.3.4:8545   | Ethereum Execution Node endpoint. Accepts a single URL or a comma-separated list (e.g. `http://primary:8545,http://backup:8545`). Each request tries the endpoints in order; on a 5xx, network or timeout error the daemon moves to the next URL. 4xx and validation errors are terminal (not retried across endpoints). Voluntary-exit submissions are an exception — they broadcast to every CL URL in parallel. **Cross-endpoint consistency.** At startup the daemon issues `eth_chainId` to every EL URL and `/eth/v1/config/deposit_contract` to every CL URL, and refuses to boot if the chain ids disagree. Misconfigured mixed-network URLs (e.g. mainnet + testnet pasted together) are caught at boot, not in production.        |
...
| STAKING_MODULE_IDENTIFIERS                    | No       | [1,2,3]               | Alternative to STAKING_MODULE_ID. Array of Staking Module IDs to handle exits for multiple staking modules simultaneously                                                                                                                                                       |
...
| VOTING_EVENTS_FRAME_BLOCKS                    | No       | 216000                | How many blocks of Easy-Track motion events to look back through when security-verifying voluntary-exit requests. Default ~30 days. Lower values reduce RPC load but may cause some legitimate exit requests to fail security verification if the corresponding motion is older. |
...
| LOGGER_SECRETS                                | No       | ["MESSAGES_PASSWORD"] | JSON string array of either env var keys to sanitize in logs or exact values. The sanitizer matches literal substrings; when `EXECUTION_NODE` or `CONSENSUS_NODE` is a comma-separated list, only the full string is redacted by default. To redact each URL individually in fallback log lines, list each URL as its own entry (e.g. `["http://primary:8545","http://backup:8545"]`). |
