...
| LOGGER_SECRETS                                | No       | ["MESSAGES_PASSWORD"] | JSON string array of either env var keys to sanitize in logs or exact values. The sanitizer matches literal substrings. When `LOGGER_SECRETS` includes `EXECUTION_NODE` or `CONSENSUS_NODE`, comma-separated RPC URLs are also redacted individually after config normalization. |
