...
Requests can optionally be authenticated with a bearer token by setting either the WEBHOOK_TOKEN variable or the WEBHOOK_TOKEN_FILE variable pointing to a file with the token inside. The token is sent in the `Authorization: Bearer <token>` header. Set WEBHOOK_HEADER to send the token in a custom header instead (e.g. `X-Api-Key: <token>`), in which case the raw token is sent without the `Bearer` prefix.
...
| SUBMIT_TX_HASH_ALLOWLIST                      | No       | ["0x123"]             | Exact hashes of final non-Oracle transactions that call `submitExitRequestsData`. The Ejector reconstructs the signed transaction hash and accepts only validator pubkeys present in its authenticated calldata. Leave empty to reject this path. |
...
| LOAD_LOGS_STEP                                | No       | 10000                 | Maximum block range per eth_getLogs request. Wider ranges are split into multiple requests. Lower this value if your node or RPC provider limits event query ranges                                                                                                             |
...
| HTTP_HOST                                     | No       | all interfaces        | Network interface to bind the HTTP server (metrics and health check) to. Accepts IPv4 (e.g. `127.0.0.1`, `0.0.0.0`) or IPv6 (e.g. `::1`, `::`). Set to a private interface address to avoid exposing the server publicly. If unset, Node.js listens on all interfaces (`::` on IPv6-capable hosts, otherwise `0.0.0.0`) |
...
| WEBHOOK_HEADER                                | No       | Authorization         | Header the webhook token is sent in. With the default Authorization header the token is sent as `Bearer <token>`, with a custom header (e.g. X-Api-Key) the raw token is sent                                                                                                   |
