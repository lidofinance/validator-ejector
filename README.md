...
| MESSAGES_PASSWORD_FILE     | No       | password_inside.txt   | Path to a file with password inside to decrypt exit messages with. Needed only if you have encrypted exit messages. If used, exact value of the password needs to be added to LOGGER_SECRETS in order to be sanitized correctly                         |
