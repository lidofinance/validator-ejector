...
| STAKING_MODULE_ID                             | Yes*     | 123                   | Staking Module ID for which operator ID is set, currently only one exists - ([NodeOperatorsRegistry](https://github.com/lidofinance/lido-dao#contracts)) with id `1`. Not required when `EJECTOR_SCOPE` is set.                                                                 |
| EJECTOR_SCOPE                                 | No       | {"1":[123],"4":[0,1]} | Module-to-operators mapping for multi-module setups. If set, it replaces `STAKING_MODULE_ID`, `OPERATOR_ID`, and `OPERATOR_IDENTIFIERS`.                                                                                                                                       |
| OPERATOR_ID                                   | Yes*     | 123                   | Operator ID in the Node Operators registry, easiest to get from Operators UI: [Hoodi](https://operators-hoodi.testnet.fi/)/[Mainnet](https://operators.lido.fi). Not required when `EJECTOR_SCOPE` is set.                                                                     |
| OPERATOR_IDENTIFIERS                          | No       | [0,1,2]               | Alternative to OPERATOR_ID for multiple operators within one `STAKING_MODULE_ID`.                                                                                                                                                                                               |
